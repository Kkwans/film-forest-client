...
export default function MovieCard({
  ...
  const genreArr = Array.isArray(genre) ? genre : (genre ? [genre] : []);
  // Normalize region to display string (filter empty)
  // Normalize region: parse JSON string or array
  let regionArr: string[] = [];
  if (Array.isArray(region)) {
    regionArr = region.filter(Boolean);
  } else if (typeof region === 'string' && region) {
    try { const parsed = JSON.parse(region); regionArr = Array.isArray(parsed) ? parsed : [region]; } catch { regionArr = [region]; }
  }
  ...
  const cleanTitle = title.replace(/\(\d{4}\)\s*$/, '').trim();
