  ...
  const fetchData = useCallback(async (p: number, g: string, r: string, y: string, s: string, yf?: string, yt?: string) => {
    ...
    try {
      ...
      setItems(raw.map((m: any) => ({
        ...
        year: m.year || 0,
        region: (() => { try { const r = typeof m.region === 'string' ? JSON.parse(m.region) : m.region; return Array.isArray(r) ? r : [m.region]; } catch { return m.region ? [m.region] : []; } })(),
        rating: m.scoreDouban || m.scoreImdb || undefined,
        genre: Array.isArray(m.genre) ? m.genre : (() => { try { return JSON.parse(m.genre); } catch { return []; } })(),
