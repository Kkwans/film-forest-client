    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row gap-6">
        ...
        <div className="flex-1 flex flex-col gap-3 min-w-0">
          <h1 className="text-2xl md:text-3xl font-bold" style={{ color: 'var(--text-primary)' }}>
            {movie.title.replace(/\(\d{4}\)\s*$/, '').trim()} {movie.year && <span className="text-lg font-normal" style={{ color: 'var(--text-muted)' }}>({movie.year})</span>}
          </h1>
      ...
      {movie.summary && (
        <section className="rounded-xl p-5 border" style={{ backgroundColor: 'var(--bg-secondary)', borderColor: 'var(--border-color)' }}>
          ...
          <p className={`text-sm leading-relaxed ${synopsisExpanded ? '' : 'line-clamp-3'}`} style={{ color: 'var(--text-secondary)' }}>{movie.summary.replace(/\[\]\s*$/, '').trim()}</p>
          {movie.summary.replace(/\[\]\s*$/, '').trim().length > 200 && (
