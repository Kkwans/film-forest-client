    ...
    try {
      ...
      if (d && d.id) {
        setItem({
          ...
          region: Array.isArray(d.region) ? d.region.join(' / ') : (d.region || ''),
          rating: d.scoreDouban, summary: d.storyline,
          status: d.status === 1 ? '连载中' : '已完结',
          totalEpisode: d.totalEpisode, genre: Array.isArray(d.genre) ? d.genre : (d.genre ? JSON.parse(d.genre) : []), director: Array.isArray(d.director) ? d.director : (d.director ? JSON.parse(d.director) : []),
        });
      }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row gap-6">
        ...
        <div className="flex-1 flex flex-col gap-3 min-w-0">
          <h1 className="text-2xl md:text-3xl font-bold" style={{ color: 'var(--text-primary)' }}>{item.title.replace(/\(\d{4}\)\s*$/, '').trim()}{item.year && <span className="text-lg font-normal ml-2" style={{ color: 'var(--text-muted)' }}>({item.year})</span>}</h1>
      ...
      {item.summary && (
        <section className="rounded-xl p-5 border" style={{ backgroundColor: 'var(--bg-secondary)', borderColor: 'var(--border-color)' }}>
          ...
          <p className={`text-sm leading-relaxed ${synopsisExpanded ? '' : 'line-clamp-3'}`} style={{ color: 'var(--text-secondary)' }}>{item.summary.replace(/\[\]\s*$/, '').trim()}</p>
          {item.summary.replace(/\[\]\s*$/, '').trim().length > 200 && (
