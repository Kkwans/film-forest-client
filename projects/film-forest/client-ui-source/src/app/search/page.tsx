...
function parseRegion(region: string | string[] | undefined): string {
  if (!region) return '';
  if (Array.isArray(region)) return region.join('/');
  try { const p = JSON.parse(region); return Array.isArray(p) ? p.join('/') : region; } catch { return region; }
}
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {filteredResults.map(item => {
              ...
              const regionStr = parseRegion(item.region);
                ...
                <Link key={`${item.type}-${item.id}`} href={href} className="flex gap-3 p-3 rounded-xl border transition-colors hover:shadow-md" style={{ backgroundColor: 'var(--bg-secondary)', borderColor: 'var(--border-color)' }}>
                  ...
                  <div className="flex-1 min-w-0 flex flex-col gap-1">
                    <div className="flex items-center gap-2">
                      <h3 className="font-bold text-sm md:text-base truncate" style={{ color: 'var(--text-primary)' }}>{cleanTitle(item.title)}</h3>
