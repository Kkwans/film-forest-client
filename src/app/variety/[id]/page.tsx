...
export default function VarietyDetailPage() {
  ...
  useEffect(() => {
    ...
    statusApi.get(id, 'variety').then(res => {
  ...
  return (
          ...
          <>
            <button onClick={async () => {
              ...
              try {
                ...
                if (wantList) { await listApi.addItem(wantList.id, { movieId: id, contentType: 'variety' }); setMovieStatus(prev => ({ ...prev, want_to_watch: true })); showToast('已加入想看', 'success'); }
              } catch {}
    ...
    <WatchedModal open={watchedOpen} onClose={() => setWatchedOpen(false)} movieId={id} contentType="variety" movieTitle={item?.title} />
    </>
  );
}
