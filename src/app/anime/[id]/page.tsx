...
export default function AnimeDetailPage() {
  ...
  useEffect(() => {
    ...
    statusApi.get(id, 'anime').then(res => {
  ...
  return (
          ...
          <>
            <button onClick={async () => {
              ...
              try {
                ...
                if (wantList) { await listApi.addItem(wantList.id, { movieId: id, contentType: 'anime' }); setMovieStatus(prev => ({ ...prev, want_to_watch: true })); showToast('已加入想看', 'success'); }
              } catch {}
    ...
    <WatchedModal open={watchedOpen} onClose={() => setWatchedOpen(false)} movieId={id} contentType="anime" movieTitle={item?.title} />
    </>
  );
}
